...
import {
  loginErrorPlaceholders, errorMessages
...
And(
  "I provide valid data and {string} and used {string}",
  (accountType, emailAddress) => {
    ...
    cy.get(createDetails.eMail).type(emailAddress);
...
Then("User is not successfully created and info message is displayed", () => {
    ...
    cy.get(loginErrorPlaceholders.alertBox).contains(errorMessages.usedEmail);
